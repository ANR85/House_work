...
// Helper central para todas as chamadas ao Supabase
...
const api = {
  ...
  async getFamilies(token) {
    return db("GET", "/rest/v1/families?select=id", token);
  },
  async createFamily(token, userId) {
    return db("POST", "/rest/v1/families", token, { user_id: userId });
  },
  async getChildren(token, familyId) {
    return db("GET", `/rest/v1/children?family_id=eq.${familyId}&order=position.asc`, token);
  },
  async createChild(token, body) {
    return db("POST", "/rest/v1/children", token, body);
  },
  async updateChild(token, id, body) {
    return db("PATCH", `/rest/v1/children?id=eq.${id}`, token, body);
  },
  async deleteChild(token, id) {
    return db("DELETE", `/rest/v1/children?id=eq.${id}`, token);
  },
  async getTasks(token, childIds) {
    return db("GET", `/rest/v1/tasks?child_id=in.(${childIds.join(",")})&order=position.asc`, token);
  },
  async createTask(token, body) {
    return db("POST", "/rest/v1/tasks", token, body);
  },
  async updateTask(token, id, body) {
    return db("PATCH", `/rest/v1/tasks?id=eq.${id}`, token, body);
  },
  async deleteTask(token, id) {
    return db("DELETE", `/rest/v1/tasks?id=eq.${id}`, token);
  },
  async getRecords(token, childIds, date) {
    return db("GET", `/rest/v1/daily_records?child_id=in.(${childIds.join(",")})&date=eq.${date}`, token);
  },
  async getAllRecords(token, childIds) {
    return db("GET", `/rest/v1/daily_records?child_id=in.(${childIds.join(",")})&order=date.asc`, token);
  },
  async upsertRecord(token, body) {
    const r = await fetch(`${SUPA_URL}/rest/v1/daily_records?on_conflict=child_id,date,task_id`, {
      method: "POST",
      headers: {
        "apikey": SUPA_KEY,
        "Authorization": `Bearer ${token}`,
  ...
  async deleteRecords(token, childIds, date) {
    return db("DELETE", `/rest/v1/daily_records?child_id=in.(${childIds.join(",")})&date=eq.${date}`, token);
  },
};
...
const CSS = `
  ...
  * { box-sizing: border-box; }
  ...
  .child-pill { border:2px solid transparent;border-radius:99px;padding:4px 14px;cursor:pointer;font-family:'Nunito',sans-serif;font-weight:800;font-size:.8rem;transition:all .15px; }
...
function AuthScreen({ onAuth }) {
  ...
  const handle = async () => {
    ...
    try {
      if (mode === "signup") {
        if (password !== confirm) { setError("As senhas não coincidem."); setLoad(false); return; }
        if (password.length < 6)  { setError("Senha deve ter ao menos 6 caracteres."); setLoad(false); return; }
        const res = await api.signUp(email, password);
        if (res.error) { setError(res.error.message || "Erro ao criar conta."); setLoad(false); return; }
        const res2 = await api.signIn(email, password);
        if (!res2.access_token) { setError("Conta criada! Faça login."); setMode("login"); setLoad(false); return; }
        onAuth(res2.access_token);
      } else {
        const res = await api.signIn(email, password);
        if (!res.access_token) { setError("Email ou senha incorretos."); setLoad(false); return; }
        ...
        <div style={{ padding:"1.8rem 2rem" }}>
          ...
          <div style={{ display:"flex",flexDirection:"column",gap:".8rem" }}>
            ...
            <div>
              ...
              <input className="inp" type="password" placeholder="Mínimo 6 caracteres" value={password} onChange={e=>setPass(e.target.value)} onKeyDown={e=>e.key==="Enter"&&handle()} />
            </div>
            {mode==="signup" && (
              <div>
                ...
                <input className="inp" type="password" placeholder="Repita a senha" value={confirm} onChange={e=>setConf(e.target.value)} onKeyDown={e=>e.key==="Enter"&&handle()} />
              </div>
            )}
            ...
            <button className="btn" onClick={handle} disabled={loading||!email||!password} style={{ background:"linear-gradient(135deg,#667eea,#764ba2)",color:"white",padding:".75rem",fontSize:"1rem",marginTop:".2rem",display:"flex",alignItems:"center",justifyContent:"center",gap:".6rem" }}>
...
function AddChildModal({ token, familyId, onClose, onSaved, editChild }) {
  ...
  const save = async () => {
    ...
    try {
      ...
      if (editChild) {
        const res = await api.updateChild(token, editChild.id, body);
        if (res?.error) { setError("Erro ao salvar. Tente novamente."); setLoading(false); return; }
      } else {
        const res = await api.createChild(token, { ...body, family_id: familyId, position: Date.now() });
        if (res?.error || (Array.isArray(res) && res[0]?.error)) { setError("Erro ao criar criança. Tente novamente."); setLoading(false); return; }
      }
      onSaved();
        ...
        <div style={{ padding:"1.3rem 1.4rem",display:"flex",flexDirection:"column",gap:"1rem" }}>
          ...
          <div>
            ...
            <div style={{ display:"flex",flexWrap:"wrap",gap:"5px" }}>
              {EMOJI_OPTIONS.map(em => <button key={em} onClick={()=>setEmoji(em)} style={{ background:em===emoji?COLOR_OPTIONS[colorIdx].light:"#F5F5F5",border:`2px solid ${em===emoji?COLOR_OPTIONS[colorIdx].color:"#EEE"}`,borderRadius:"8px",padding:"4px 7px",cursor:"pointer",fontSize:"1.2rem" }}>{em}</button>)}
            </div>
          </div>
          <div>
            ...
            <div style={{ display:"flex",gap:"7px",flexWrap:"wrap" }}>
              {COLOR_OPTIONS.map((c,i) => <div key={i} onClick={()=>setColorIdx(i)} style={{ width:30,height:30,borderRadius:"50%",background:c.color,cursor:"pointer",border:`3px solid ${i===colorIdx?"#333":"transparent"}`,transition:"all .15s",transform:i===colorIdx?"scale(1.2)":"scale(1)" }} />)}
            </div>
          </div>
          {error && <div style={{ background:"#FFE8E8",border:"1.5px solid #FF6B6B",borderRadius:"10px",padding:".6rem .9rem",color:"#C0392B",fontSize:".85rem",fontWeight:700 }}>⚠️ {error}</div>}
...
function EditTasksModal({ token, child, onClose, onSaved }) {
  ...
  useEffect(() => {
    api.getTasks(token, [child.id]).then(data => { setTasks(Array.isArray(data)?data:[]); setLoad(false); });
  }, []);

  const add = async (lbl, ico) => {
    if (!lbl.trim()) return;
    const pos = (tasks.length ? Math.max(...tasks.map(t=>t.position||0)) : 0) + 1000;
    const res = await api.createTask(token, { child_id:child.id, label:lbl.trim(), icon:ico, position:pos });
    const newTask = Array.isArray(res) ? res[0] : res;
    if (newTask?.id) setTasks(p => [...p, newTask]);
    setLabel(""); setIcon("✨"); setPicker(false);
  };

  const remove = async (id) => { await api.deleteTask(token, id); setTasks(p=>p.filter(t=>t.id!==id)); };
  const move = (idx, dir) => setTasks(p => { const a=[...p],s=idx+dir; if(s<0||s>=a.length)return a; [a[idx],a[s]]=[a[s],a[idx]]; return a; });
  ...
  const saveAll = async () => {
    setSaving(true);
    await Promise.all(tasks.map((t,i) => api.updateTask(token, t.id, { label:t.label, icon:t.icon, position:(i+1)*1000 })));
      ...
      <div style={{ background:"white",borderRadius:"26px",width:"100%",maxWidth:"490px",maxHeight:"92vh",overflowY:"auto",boxShadow:"0 24px 70px rgba(0,0,0,.35)",animation:"slideUp .25s cubic-bezier(.34,1.56,.64,1)" }}>
        <div style={{ background:`linear-gradient(135deg,${child.color},${child.color}aa)`,borderRadius:"26px 26px 0 0",padding:"1.1rem 1.4rem",display:"flex",alignItems:"center",justifyContent:"space-between",position:"sticky",top:0,zIndex:10 }}>
          <div style={{ display:"flex",alignItems:"center",gap:".7rem" }}>
            ...
            <div><div style={{ fontFamily:"'Fredoka One',cursive",fontSize:"1.15rem",color:"white" }}>{child.name}</div><div style={{ color:"rgba(255,255,255,.8)",fontSize:".75rem",fontWeight:700 }}>Editar tarefas</div></div>
          </div>
          <button onClick={onClose} style={{ background:"rgba(255,255,255,.25)",border:"none",borderRadius:"50%",width:34,height:34,cursor:"pointer",color:"white",fontSize:"1rem",display:"flex",alignItems:"center",justifyContent:"center" }}>✕</button>
        </div>
            ...
            <>
              ...
              <div style={{ borderTop:"2px dashed #EEE",paddingTop:"1rem",marginBottom:"1rem" }}>
                ...
                <div style={{ display:"flex",gap:".45rem",marginBottom:".5rem" }}>
                  <button onClick={()=>setPicker(v=>!v)} style={{ background:"#F0F0F0",border:"2px solid #DDD",borderRadius:"10px",padding:".35rem .55rem",fontSize:"1.2rem",cursor:"pointer",flexShrink:0 }}>{icon}</button>
                ...
                {showPicker && <div style={{ background:"#F8F8F8",borderRadius:"12px",padding:".55rem",display:"flex",flexWrap:"wrap",gap:"4px",marginBottom:".5rem" }}>{ICONS.map(ic=><button key={ic} onClick={()=>{setIcon(ic);setPicker(false);}} style={{ background:ic===icon?child.light:"white",border:`2px solid ${ic===icon?child.color:"#EEE"}`,borderRadius:"8px",padding:"4px 6px",cursor:"pointer",fontSize:"1.05rem" }}>{ic}</button>)}</div>}
                ...
                <div style={{ display:"flex",flexWrap:"wrap",gap:".35rem" }}>
                  {SUGGESTIONS.filter(s=>!tasks.find(t=>t.label===s.label)).slice(0,9).map(s=><button key={s.label} onClick={()=>add(s.label,s.icon)} style={{ background:"#F2F2F2",border:"1.5px solid #E5E5E5",borderRadius:"99px",padding:"3px 10px",fontSize:".75rem",fontFamily:"'Nunito',sans-serif",fontWeight:700,cursor:"pointer",color:"#555" }}>{s.icon} {s.label}</button>)}
                </div>
              </div>
              <div style={{ display:"flex",gap:".6rem",justifyContent:"flex-end" }}>
                <button className="btn" onClick={onClose} style={{ background:"#EEE",color:"#666",padding:".55rem 1.1rem",fontSize:".88rem" }}>Cancelar</button>
                <button className="btn" onClick={saveAll} disabled={saving} style={{ background:`linear-gradient(135deg,${child.color},${child.color}bb)`,color:"white",padding:".55rem 1.4rem",fontSize:".88rem",display:"flex",alignItems:"center",gap:".5rem" }}>{saving?<Spinner size={16}/>:"✅ Salvar"}</button>
              </div>
            </>
          )}
        </div>
...
function HistoryView({ token, children, onBack }) {
  ...
  useEffect(() => {
    if (!children.length) { setLoad(false); return; }
    api.getAllRecords(token, children.map(c=>c.id)).then(data => { setRecords(Array.isArray(data)?data:[]); setLoad(false); });
  }, []);
  ...
  const chartData = dates.map(date => {
    ...
    children.forEach(c => { const s=stats[date]?.[c.id]; row[`c_${c.id}`]=s&&s.total>0?Math.round(s.done/s.total*100):null; });
    return row;
  });

  const getSummary = cid => Object.entries(stats).sort(([a],[b])=>a>b?1:-1).map(([date,ds]) => {
    const s=ds[cid]; if(!s||!s.total) return null;
    const pct=Math.round(s.done/s.total*100); return { date, pct, done:s.done, total:s.total };
  }).filter(Boolean);

  const getAvg    = cid => { const r=getSummary(cid); return r.length?Math.round(r.reduce((s,x)=>s+x.pct,0)/r.length):0; };
  const getBest   = cid => getSummary(cid).reduce((b,r)=>r.pct>b?r.pct:b,0);
      ...
      {loading ? <div style={{ textAlign:"center",padding:"4rem" }}><Spinner /></div> : Object.keys(stats).length===0 ? (
        ...
        <>
          <div style={{ display:"flex",gap:".5rem",flexWrap:"wrap",marginBottom:"1.2rem",alignItems:"center" }}>
            ...
            {children.map(c=><span key={c.id} className="child-pill" onClick={()=>setSelected(selected===c.id?null:c.id)} style={{ background:selected===c.id?c.color:"transparent",color:selected===c.id?"white":"rgba(255,255,255,.7)",borderColor:selected===c.id?c.color:"rgba(255,255,255,.3)" }}>{c.emoji} {c.name}</span>)}
          </div>
          <div style={{ display:"flex",gap:".4rem",marginBottom:"1.4rem" }}>
            {[["chart","📈 Gráfico"],["table","📋 Tabela"]].map(([v,l])=><button key={v} className="tab-btn" onClick={()=>setView(v)} style={{ background:view===v?"white":"rgba(255,255,255,.12)",color:view===v?"#333":"white" }}>{l}</button>)}
          </div>
          {view==="chart" && (
            <div style={{ background:"rgba(255,255,255,.07)",border:"1px solid rgba(255,255,255,.12)",borderRadius:"20px",padding:"1.2rem",marginBottom:"1.5rem" }}>
              ...
              <ResponsiveContainer width="100%" height={280}>
                <LineChart data={chartData} margin={{ top:5,right:10,left:-20,bottom:5 }}>
                  ...
                  <Tooltip contentStyle={{ background:"#1a1a2e",border:"1px solid rgba(255,255,255,.2)",borderRadius:"12px",fontFamily:"'Nunito',sans-serif" }} labelStyle={{ color:"white",fontWeight:800 }} formatter={(v,n)=>[v!=null?`${v}%`:"—",n]} />
                ...
                <tbody>
                  {Object.keys(stats).sort((a,b)=>b>a?1:-1).map((date,ri)=>(
                    <tr key={date} style={{ background:ri%2===0?"transparent":"rgba(255,255,255,.03)" }}>
                      ...
                      {toShow.map(c=>{ const s=stats[date]?.[c.id]; if(!s||!s.total) return <td key={c.id} style={{ textAlign:"center",padding:".5rem .7rem",color:"rgba(255,255,255,.2)" }}>—</td>; const pct=Math.round(s.done/s.total*100); const medal=getMedal(pct); return <td key={c.id} style={{ textAlign:"center",padding:".5rem .7rem" }}><span style={{ fontWeight:800,color:pct===100?"#FFD700":pct>=80?"#C0C0C0":pct>=60?"#CD7F32":"rgba(255,255,255,.6)" }}>{medal?.icon} {pct}%</span><div style={{ fontSize:".7rem",color:"rgba(255,255,255,.35)" }}>{s.done}/{s.total}</div></td>; })}
                    </tr>
                  ))}
                </tbody>
              </table>
          ...
          <div style={{ display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(220px,1fr))",gap:".8rem" }}>
            {toShow.map((c,i)=>{
              const avg=getAvg(c.id), best=getBest(c.id), streak=getStreak(c.id), days=getSummary(c.id).length;
              return (
...
export default function App() {
  ...
  const handleLogout = async () => { await api.signOut(token); sessionStorage.removeItem("supa_token"); setToken(""); setFamilyId(null); setChildren([]); setLoading(true); };
  ...
  const loadAll = useCallback(async (tok) => {
    ...
    try {
      ...
      // Busca ou cria família
      let fams = await api.getFamilies(tok);
      if (!Array.isArray(fams) || !fams.length) {
        const created = await api.createFamily(tok, userId);
        fams = Array.isArray(created) ? created : [created];
      }
      if (!fams.length || !fams[0]?.id) { setLoading(false); return; }
      const fid = fams[0].id;
      setFamilyId(fid);

      // Busca crianças
      const kids = await api.getChildren(tok, fid);
      const kidsArr = Array.isArray(kids) ? kids : [];
      setChildren(kidsArr);

      if (kidsArr.length) {
        const ids = kidsArr.map(c=>c.id);
        const allTasks = await api.getTasks(tok, ids);
        const taskMap = {};
        kidsArr.forEach(c => { taskMap[c.id]=[]; });
        (Array.isArray(allTasks)?allTasks:[]).forEach(t => { if(taskMap[t.child_id]) taskMap[t.child_id].push(t); });
        setTasksByChild(taskMap);

        const recs = await api.getRecords(tok, ids, TODAY);
        const chk = {};
        kidsArr.forEach(c => { chk[c.id]={}; });
        (Array.isArray(recs)?recs:[]).forEach(r => { if(chk[r.child_id]) chk[r.child_id][r.task_id]=r.completed; });
        setChecked(chk);
        setSavedToday(Array.isArray(recs) && recs.length > 0);
      }
  ...
  const toggle = async (cid, tid) => {
    ...
    await api.upsertRecord(token, { child_id:cid, task_id:tid, date:TODAY, completed:!cur });
    setSavedToday(true);
  };
  ...
  const handleSaveDay = async () => {
    setSavingDay(true);
    const ops = [];
    children.forEach(c => { (tasksByChild[c.id]||[]).forEach(t => { ops.push(api.upsertRecord(token, { child_id:c.id, task_id:t.id, date:TODAY, completed:!!checked[c.id]?.[t.id] })); }); });
  ...
  const handleReset = async () => {
    ...
    if (ids.length) await api.deleteRecords(token, ids, TODAY);
    const chk = {}; children.forEach(c=>{chk[c.id]={};});
    setChecked(chk); setSavedToday(false); setResetConfirm(false);
  };

  const handleDeleteChild = async cid => { await api.deleteChild(token, cid); setDeleteConfirm(null); loadAll(token); };
              ...
              <div style={{ background:`linear-gradient(135deg,${child.color},${child.color}cc)`,padding:"1rem 1.1rem .9rem",display:"flex",alignItems:"center",justifyContent:"space-between" }}>
                <div style={{ display:"flex",alignItems:"center",gap:".55rem",minWidth:0 }}>
                  ...
                  <div style={{ minWidth:0 }}>
                    <div style={{ fontFamily:"'Fredoka One',cursive",fontSize:"1.1rem",color:"white",overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap",maxWidth:"130px" }}>{child.name}</div>
